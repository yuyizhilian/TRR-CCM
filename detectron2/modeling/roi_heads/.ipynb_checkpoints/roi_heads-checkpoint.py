...
class Res5ROIHeads(ROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        # self.reduce_channel = nn.Conv2d(in_channels=1024, out_channels=self.deform_output_channels, kernel_size=1,
        #                                 stride=1, padding=0)
    ...
    def _shared_roi_transform(self, features, boxes):
        x = self.pooler(features,
                        boxes)  # batch_size=4时，features[0].shape: [4, 1024, 46, 68], x的shape为[2048, 1024, 7, 7]
        # print('pooler:', x.size())
    ...
    def _enhance_feature(self, num_images, box_features, feature_pooled, pred_class_logits,
        ...
        relation = torch.empty(2, self.gcn_relation_k * num_rois, dtype=torch.long).to(
            device)  # shape: [2, 32 * 512 * batch_size = 16384] 所有的边
    ...
    def forward(self, images, features, proposals, targets=None):
        ...
        m_pred_class_logits, m_pred_proposal_deltas = self.box_predictor(att_feature_pooled)
        ...
        # pred_class_logits = pred_class_logits * (1 - self.deform_weight) + m_pred_class_logits * self.deform_weight
        if self.train_gcn:
            pred_class_logits, pred_proposal_deltas = self._enhance_feature(
                num_images, box_features, feature_pooled, pred_class_logits, pred_proposal_deltas)
