    ...
    def __init__(self, cfg):
        ...
        if cfg.MODEL.TRAIN_GCN:
            ...
            for p in self.roi_heads.pooler.parameters():
                p.requires_grad = False
            for p in self.roi_heads.bf_transformer.parameters():
                p.requires_grad = False
            # for p in self.roi_heads.af_mamba.parameters():
            #     p.requires_grad = False
            # for p in self.roi_heads.roi_box_predictor.parameters():
            #     p.requires_grad = False
            for p in self.roi_heads.reduce_channel_1.parameters():
                p.requires_grad = False
            for p in self.roi_heads.reduce_channel_2.parameters():
                p.requires_grad = False
            for p in self.roi_heads.roi_transformer.parameters():
                p.requires_grad = False
            for p in self.roi_heads.conv1_1.parameters():
                p.requires_grad = False
